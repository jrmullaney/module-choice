...
- `Data/Regs/regs.txt` — raw programme regulations from admin (one programme so far: MPSU022 BSc Physics)
- `Data/Regs/regs_processed.txt` — processed regulations in TSV format (to be built); loaded into DB by `build_db.py`; columns: `programme`, `level`, `group`, `rule`, `min_credits`, `max_credits`, `semester`, `module` — `rule` is `mandatory` or `choose`; `min_credits`/`max_credits` are credit bounds for `choose` groups (blank for mandatory); `semester` is `AUT`, `SPR`, `YR`, or blank
...
10. **Programme regulations / core module data**: raw regs received for MPSU022; agreed processed TSV format (`Data/Regs/regs_processed.txt`) with columns `programme`, `level`, `group`, `rule`, `min_credits`, `max_credits`, `semester`, `module`. Next: build `regs_processed.txt`, load into DB via `build_db.py` (new `programme_regulations` table replacing/extending `programme_modules`), add validation in `validate_choices.py`, add **Programmes** admin page to UI.
